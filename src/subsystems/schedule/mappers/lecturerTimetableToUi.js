 ...
 * @param {string} ruDate
 */
export function lecturerTimetableToDayList(timetable, ruDate) {
  if (!timetable) return [];
  ...
  const list = day.lessons.map((l) => {
    const groups = Array.isArray(l.groups) ? l.groups.map((g) => `Гр. ${g.name}`) : [];
    return {
      id: l.lesson_id,
      ...
      groups,
      room: l.location,
      attendanceConfirmed: !!l.is_confirmed,
      isPast: false,
      isCurrent: false,
 ...
 * @param {any} timetable
 * @param {string} startRuDate Any date inside the range (will be normalized to Monday)
 * @param {number} daysCount 6 for week (Mon-Sat), 12 for two weeks
 */
export function lecturerTimetableToMatrixSchedule(timetable, startRuDate, daysCount) {
  ...
  // Build per-day lesson list in RU date keys for easier lookup.
  ...
  for (const day of weekDays) {
    ...
    for (const time of timeSlots) {
      ...
      // Merge by subject+type+location at same time
      const merged = new Map();
      for (const l of lessonsAtTime) {
        const mergeKey = `${l.subject}|${l.lesson_type}|${l.location}`;
        const existing = merged.get(mergeKey);

        const groupNames = Array.isArray(l.groups) ? l.groups.map((g) => g.name) : [];
        ...
        if (!existing) {
          merged.set(mergeKey, {
            ...
            time: `${l.start_at} - ${l.end_at}`,
            date: day.date,
            isConfirmed: !!l.is_confirmed,
            lessonIds: [l.lesson_id],
          });
        } else {
          existing.groups = Array.from(new Set([...existing.groups, ...groupNames])).sort();
          existing.isConfirmed = existing.isConfirmed && !!l.is_confirmed;
