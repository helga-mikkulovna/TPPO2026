import { getGroupLessons } from '../api/timetable';
...
export const groupTimetableStore = {
  /** @param {() => void} listener */
  subscribe(listener) {
  ...
  async load(query) {
    ...
    try {
      const data = await getGroupLessons(query, { signal: controller.signal });
